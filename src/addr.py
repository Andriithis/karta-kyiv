...
TYPES = [
 # «вулицею» — орудний: «рухаючись вулицею Будівельників, 29»
 (r'вул(?:иц(?:ею|[іяею]))?\.?', 'вул.'),
 # «пр-т», «пр-ту», «бул.» — так пишуть у протоколах; без них «бул.Чоколівський,
 # 19» не знаходився зовсім (вибірка 23.09, ZVIT-TEKSTY-1.md, №3). «пр.» —
 # найчастіше скорочення проспекту в протоколах: на першій частині проходу
 # 944 адреси «по пр. Науки, 40-А» лишилися без місця. Після типу завжди йде
 # назва з великої літери, тож «пр. № 3/…» (провадження) сюди не потрапляє.
 # «п-ту», «пр.-ту» — теж проспект (вибірка none після v2, 24.09)
 (r'просп(?:ект(?:ом|[уі])?)?\.?|пр-т\w*\.?|пр\.-т\w*\.?|п-т\w*\.?|пр\.', 'просп.'),
 (r'бульв(?:ар[уі]?)?\.?|б-р\w*\.?|бул\.', 'бульв.'),
 (r'проїзд\w*', 'проїзд'),
 (r'пров(?:улок|улку|ул\.|\.)?', 'пров.'),
 (r'площ[аіі]|пл\.', 'пл.'),
 (r'шосе', 'шосе'),
 (r'наб(?:ережн\w*)?\.?', 'наб.'),
 (r'узвіз|узвоз\w+', 'узвіз'),
 (r'[Мм]айдан\w*', 'майдан'),      # «Майдан Незалежності, 2»
 (r'[Аа]ле[яї]', 'алея'),        # «на Алеї Героїв Крут» пишуть і з великої
...
# Через пробіл — лише велика літера: мала після пробілу — це прийменник, і
# «вул. Кобелєва, 1/5 у м.Києві» ставало будинком «1/5У», а «11 в м.Києві» —
# «11В», чужим будинком (рішення 25.09, PLAN-ZAHALNYI.md, А4а). Літера не
# може бути й початком слова: «5 Вул…» — не «5В».
# Пробіл перед дробом — усередині групи дробу: вільний \s* одразу після цифр
# з'їдав пробіл, і «22 в» знову читалося як «22в» впритул.
HOUSE = (r"(\d{1,4}(?:\s*[-/]\s*\d{1,3})?"
         r"(?:\s*-\s*[А-Яа-яA-Za-z]|\s+[А-ЯA-Z]|[А-Яа-яA-Za-z])?(?![А-Яа-яA-Za-z]))")
...
# Орудний і родовий теж: «рухаючись Воскресенським проспектом», «біля
# будинку 5 Наддніпрянського шосе».
_PB_TYPES = (r"вулиц(?:ею|[іяею])|проспект(?:ом|[уі])?|бульвар(?:ом|[уі])?|провулк?ом|провулок|провулку|"
             r"набережн\w+|площ(?:ею|[аіі])|пл\.|[Шш]осе|узвіз|узвоз\w+|проїзд\w*|дорог\w+|дороз\w+|"
             # мости й шляхопроводи — місце ДТП: «на Дарницькому мосту»
             r"мост\w*|міст|шляхопровод\w*|шляхопровід")
# Слово назви може бути подвійним через дефіс: «Брест-Литовське шосе».
_PW = r"[А-ЯІЇЄҐ][а-яіїєґ'’]*(?:-[А-ЯІЇЄҐа-яіїєґ][а-яіїєґ'’]*)?"
# Назва — лише слова з великої літери: інакше «у м. Києві на Кудрявському
# узвозі, 2» ставало вулицею «Києві на Кудрявський».
PB = re.compile(rf"\b((?:{_PW}\s+){{0,2}}{_PW})\s+({_PB_TYPES})[,\s]+(?:буд(?:инок|\.)?\s*)?№?\s*{HOUSE}\b", re.U)
# Назва перед типом, без номера: «по Кільцевій дорозі», «на Великій Кільцевій
# дорозі». Лише прикметникова назва з одного-трьох слів — інакше «Київської
# області» чи «районного суду» ставали б вулицями.
PBS = re.compile(rf"\b((?:{_PW}\s+){{0,2}}{_PW})\s+({_PB_TYPES})\b", re.U)
ADJ_END = re.compile(r"(?:ий|ій|а|е|ому|ої|ою|им|ого|ім)$")
# Адреса взагалі без типу: «за адресою: м. Київ, Житньоторська, 8», «м.Київ,
# Лютеранська, буд.3». Лише прикметникова назва одразу після «м. Київ» —
# так пишуть саме вулиці; «м. Київ, Троєщина, 5» сюди не потрапляє.
PK = re.compile(rf"\bм\.\s*Ки[їє]в\w*\s*,\s*([А-ЯІЇЄҐ][а-яіїєґ'’\-]+(?:ськ|цьк|зьк|ов|ев|ів|їв|ин|ін|н)а)"
                rf"\s*,\s*(?:буд(?:инок|\.)?\s*)?№?\s*{HOUSE}\b", re.U)
# Друкарські вади протоколів: «вул..», «вуд.», «булв.». Заміна тієї самої
# довжини — позиції згадок у тексті лишаються правильними.
TYPOS = [(re.compile(r'\b(вул|пров|просп|бульв|пл|наб)\.\.', re.U), r'\1. '),
         (re.compile(r'\bвуд\.', re.U), 'вул.'), (re.compile(r'\bбулв\.', re.U), 'бул. ')]
...
def pb_type(raw_t):
    """Тип вулиці для варіанта «назва перед типом». Кільцева дорога й
    проїзд в OSM пишуться з типом у назві, тож і тут — «вул. Кільцева
    дорога», щоб step2_geocode зіставив."""
    r = raw_t.lower()
    return ('наб.' if 'абережн' in r else 'вул.' if 'улиц' in r
            else 'просп.' if 'роспект' in r else 'бульв.' if 'ульвар' in r
            else 'пров.' if 'ровул' in r else 'пл.' if 'лощ' in r
            else 'пл.' if r == 'пл.' else 'шосе' if 'шосе' in r else 'проїзд' if 'проїзд' in r
            else 'дорога' if r.startswith('дорог') or r.startswith('дороз')
            else 'міст' if r.startswith('мост') or r == 'міст'
            else 'шляхопровід' if r.startswith('шляхопровод') or r == 'шляхопровід' else 'узвіз')


def pb_street(t, n):
    """(тип, назва) у тому вигляді, що й PA: «дорога», «міст» — частина
    назви, як і в OSM («Кільцева дорога», «Дарницький міст»)."""
    if t == 'дорога':
        return 'вул.', nominative('вул.', n, before=True) + ' дорога'
    if t in ('міст', 'шляхопровід'):
        return 'вул.', nominative('просп.', n, before=True) + ' ' + t
    return t, nominative(t, n, before=True)
...
# «відділ», але не «відділення»: відділення банку, пошти, «Нової пошти» —
# звичайне місце крадіжки чи шахрайства.
# «суд» — лише окремим словом чи «судов…»: підрядком воно ловило «Досудовим
# розслідуванням встановлено, що … за адресою» і «будучи раніше судимим» —
# 174 фабули першої частини проходу лишалися без місця.
NOISE = re.compile(r"(\bсуд(?:у|і|ом|ів)?\b|\bсудов\w*|прокурат|поліці|управлінн|відділ(?!енн)|райвідділ|адвокат|канцеляр|"
...
# Решта — звороти, у яких слово-шум стоїть поруч із місцем САМОЇ події, а
# не установи (перша частина проходу, 23.09: через них 300+ адрес лишилися
# без місця): «був зупинений працівниками поліції за адресою», «на дитячому
# майданчику», «відділення банку / пошти», «судом встановлено», «з огляду
# на викладене». Штрафний майданчик і відділ поліції як місце — лишаються шумом.
NOISE_OK = re.compile(r"огляд\w*\s+місц\w*\s+(?:події|пригоди|ДТП)|з\s+огляду\s+на|"
                      r"судом\s+встановлен\w*|встановлен\w*\s+судом|(?:в|у)\s+судовому\s+засіданні|"
...
# назва вулиці.
_ADJ = r"(?:ськ|цьк|зьк|ов|ев|єв|ів|їв|ин|ін|їн|ичн|ічн|альн|ьн|нн|ен|ан|ян|ич|к)"
_GENDER = {'вул.': 'f', 'наб.': 'f', 'пл.': 'f', 'алея': 'f', 'шосе': 'n'}


def nominative(t, n, before=False):
    """before=True — назва стоїть ПЕРЕД типом («Воскресенським проспектом»,
    «Наддніпрянського шосе»): тоді прикметник і в орудному (-им, -ою), і в
    родовому (-ого, -ої). Прізвищ у такій позиції не буває — «вулиця
    Ярослава Мудрого» пишуть з типом попереду, — тож тут родовий зводимо."""
    g = _GENDER.get(t, 'm')

    def one(m):
        stem, end = m.group(1), m.group(2)
        if end in ('ій', 'ою', 'ої') and g == 'f':
            return stem + 'а'
        if end in ('ому', 'им', 'ого') and g != 'f':
            return stem + ('е' if g == 'n' else 'ий')
        return m.group(0)
    ends = 'ій|ому|ою|ої|им|ого' if before else 'ій|ому'
    return re.sub(rf"\b([А-ЯІЇЄҐ][а-яіїєґ'’]*{_ADJ})({ends})\b", one, n)
...
def norm_house(h):
    ...
    # "буд. 6 у м. Києві" -> парсер ловить "6У". Лише впритул до цифри:
    # справжні будинки з У пишуть через дефіс («2-У»), і з них знімало саму
    # літеру, лишаючи «2-».
    h = re.sub(r'(?<=\d)У$', '', h)
    # провідні нулі: "04" -> "4"
...
_GLUED = re.compile(r"(\d{1,4}(?:/\d{1,3})?)([УВ])")


def unglue(house, text=''):
    """Номер без прилиплого прийменника — для номерів, уже витягнутих старим
    шаблоном HOUSE (у базі й у частинах проходу по текстах вони лишаються,
    доки їх не перерозібрано).

    «У» без дефіса — завжди прийменник: справжні будинки з «У» в Києві пишуть
    через дефіс («14-У»), той лишається. «В» — справжня літера будинку, тож
    знімаємо її лише тоді, коли сам текст каже «11 в м.Києві» / «11 в
    місті…»: «11 В» з великої чи «11В» впритул — будинок 11В."""
    if not house:
        return house
    m = _GLUED.fullmatch(house)
    if not m:
        return house
    base, letter = m.groups()
    if letter == 'У':
        return base
    # (?<![\w/.]) — номер окремо, а не хвіст «ОСОБА_1 в м. Києві» чи дати
    if text and re.search(rf"(?<![\w/.]){re.escape(base)}\s+в\s+(?:м\.|міст|Києв|м\s)", text):
        return base
    return house
...
def find_all(text):
    ...
    for m in PB.finditer(text):
        ...
        if not n or n.lower() in STOP or len(n) < 3: continue
        t, n = pb_street(pb_type(m.group(2)), n)
        out.append((t, n, norm_house(m.group(3)), m.start()))
...
def context_ok(text, pos, win=170, floor=0):
    ...
    # Вікно — лише в межах речення адреси: слово з попереднього речення
    # («…судом встановлено таке. 22.10.2023 … за адресою») до цієї адреси
    # не стосується, а глушило 200+ місць подій.
    floor = max(floor, sentence_start(text, pos))
    seg = NOISE_OK.sub(' ', text[max(floor, pos-win):pos])
    # «…дільничим офіцером УП ГУ НП складено протокол в тім, що 20.08.2024
    # … по вул. Коперника, 12» — хто склав протокол, стоїть до «в тім, що»;
    # місце події — після
...
WHAT = re.compile(r"(?:(?:в|у)\s+т(?:ім|ому)|про\s+те|вказано|зазначено|встановлено|вбачається)\s*,?\s*що\b",
                  re.I | re.U)
...
def mentions(text, floor):
    ...
    for m in PBS.finditer(text, floor):
        p = m.start()
        if p in houses or not clean(p):
            continue
        n = re.sub(r'\s+', ' ', m.group(1)).strip(" ,.-'`")
        if n.lower() in STOP or len(n) < 3 or not ADJ_END.search(n):
            continue
        t, n = pb_street(pb_type(m.group(2)), n)
        out.append((p, 'street', f"{t} {n}", None))
