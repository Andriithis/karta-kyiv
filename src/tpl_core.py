...
// «21 серпня 2026», а не 2026-08-21 (розд. 25, А9): ISO-дату читає машина,
// людина спотикається. Що не схоже на дату — лишається як є.
const MIS_R=['січня','лютого','березня','квітня','травня','червня','липня','серпня','вересня','жовтня','листопада','грудня'];
const fmtDate=s=>{const m=/^(\d{4})-(\d{2})-(\d{2})/.exec(s||'');
 return m?`${+m[3]} ${MIS_R[+m[2]-1]} ${m[1]}`:(s||'')};
...
function openPanel(i,keepTop){
 ...
 docsFor(i).then(cs=>{
  ...
  $('#panb').innerHTML = vis.length ? vis.map(c=>{
   ...
   return '<div class="cs">'+
    `<div class="cd" title="${esc(LAW(M.cats[c[0]]))}"><b>${esc(fmtDate(c[1]))}</b>${c[6]?' · рішення':c[2]>=0?' · '+String(c[2]).padStart(2,'0')+':00':''} · ${esc(M.cats[c[0]])}</div>`+
