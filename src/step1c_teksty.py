...
# літерою й цифрою не спрацьовує. (?!\d) після року — з тієї ж причини: з
# описки в рішенні «06.11.20524» бралося «2052» (у звітах з'явився рік 2052).
# П'ятизначний «рік» — не дата, подія лишається з датою рішення.
DATE_NUM = re.compile(r'(?<!\d)(\d{1,2})[.\s/-](\d{1,2})[.\s/-](20\d{2})(?!\d)')
DATE_TXT = re.compile(r'(?<!\d)(\d{1,2})\s+(' + '|'.join(MONTHS) + r')\s+(20\d{2})(?!\d)', re.I)
...
# ---- ФАБУЛА ДЛЯ ПОКАЗУ (рішення 24.09, після sut) ----
# Лише вирізання, що не зачіпають дію: вступ про надходження, протокольна
# рамка перед подією, службові маркери, пробіли. Формули умислу, судимість і
# дублі не чіпаємо навмисно: sut() їх вирізав, і з 21 перевіреної суті 7
# лишилися без самої дії або з обірваним реченням (ZVIT-GPT-SUT.md).
SHOW_INTRO = re.compile(r'^\s*(?:До\s+[^.]{0,120}?суд\w*[^.]{0,80}?надійш\w+|'
                        r'На\s+розгляд\s+[^.]{0,80}?надійш\w+)', re.I)
SHOW_PROT = re.compile(
    r'^\s*(?:Відповідно\s+до|Згідно\s+(?:з|із)?|Як\s+вбачається\s+(?:з|із))\s+'
    r'(?:змісту\s+|тексту\s+)?(?:даного\s+|вказаного\s+|зазначеного\s+|цього\s+)?протокол\w*'
    r'(?:\s+про\s+адміністративн\w+\s+правопорушен\w+)?'
    r'(?:\s+серії\s+[А-ЯІЇЄҐA-Z]{1,4})?(?:\s*№\s*[\w/-]+)?'
    r'(?:\s+від\s+\d{1,2}[./]\d{1,2}[./]\d{2,4}\s*(?:року|р\.)?)?\s*[,:]?\s*', re.I)
# «…відносно ОСОБА_1 було складено протокол про вчинення ним 04.10.2025 …» —
# рамку до «ним» знімаємо; учасника, якщо далі його немає, лишаємо
SHOW_PROT_ACT = re.compile(r'^.{0,350}?складено\s+протокол\w*\s+про\s+вчинення\s+(?:ним|нею)\s+', re.I)
SHOW_MARKS = [
    # ІНФОРМАЦІЯ_N одразу при учасникові — дата народження; у лапках
    # («телеграм-канал «ІНФОРМАЦІЯ_2»») — назва, її не чіпаємо
    (re.compile(r'(ОСОБА_\d+)\s*,?\s*ІНФОРМАЦІЯ_\d+\b(?!\s*[»"])\s*,?'), r'\1, '),
    (re.compile(r',?\s*(?:державн\w+\s+номерн\w+\s+знак\w*|номерн\w+\s+знак\w*|д\.?\s?н\.?\s?з\.?|н\.\s?з\.)\s*'
                r'[«"]?НОМЕР_\d+[»"]?', re.I), ''),
    (re.compile(r'\.\.(?=\s)'), '.'),
    (re.compile(r'\s+([,.;:])'), r'\1'),
    (re.compile(r'\s{2,}'), ' '),
...
# Хвости, що лишилися від межі фабули: номер наступного пункту («…доларів
# США, 1.2»), «д.н.з» без номера, учасник наступного речення без присудка
# («…1118 грн. ОСОБА_5»). Самої дії вони не несуть.
SHOW_TAIL = re.compile(r'(?:,\s*\d+(?:\.\d+)+\.?|\s*,?\s*\b(?:д\.\s?н\.\s?з|д\.?н\.?з\.|н\.\s?з)\.?|'
                       r'(?<=\.)\s+ОСОБА_\d+)\s*$', re.I)


def pokaz(fab, kd='КК'):
    """Фабула з проходу -> опис на карті й у панелі. Межа — стеля проходу
    (КУпАП 1 000, КК 2 500), лише по кінцю речення."""
    if not fab:
        return ''
    t = tidy_end(fab)
    out = []
    for s in _sentences(t):
        s = s.strip()
        if not out:
            if SHOW_INTRO.search(s) and not is_event_sentence(s):
                continue
            s = SHOW_PROT.sub('', s, count=1)
            m = SHOW_PROT_ACT.match(s)
            if m:
                who = re.search(r'ОСОБА_\d+', m.group())
                s = s[m.end():]
                if who and who.group() not in s:
                    s = who.group() + ' ' + s
            s = s[:1].upper() + s[1:]
        if s:
            out.append(s)
    t = ' '.join(out)
    for rx, rep in SHOW_MARKS:
        t = rx.sub(rep, t)
    t = SHOW_TAIL.sub('', t.strip())
    return _cut(t, CAP.get(kd, CAP['КК']))
