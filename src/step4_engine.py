...
# СКІЛЬКИ ВУЛИЦЬ ПОКАЗУВАТИ — не однакове число для всіх, а стільки, скільки
# витримує сам шар. Зміна 7 вересня: 25 на кожен шар було довільним числом.
# Там, де модель сильна (дорожній рух: події сталися на всіх 25 показаних),
# перелік можна безпечно продовжувати; там, де слабка (торгівля алкоголем:
# події сталися на 3 з 25), кожна дописана вулиця — це людина, послана
# даремно.
#
# Правило: беремо найдовший перелік із сітки, на якому ЧАСТКА ВУЛИЦЬ, ДЕ
# ПОДІЇ СПРАВДІ СТАЛИСЯ у перевірочні роки, ще не впала нижче PREC_MIN.
# Рахується на роках, яких модель не бачила, тож це не самооцінка.
# Перевірка обривається на першому провалі, а не шукає щастя далі.
NGRID    = (25, 35, 50, 70, 100, 140, 200)
# скільки «тихих» вулиць показувати на тему. Небагато навмисно: це перелік
# для превентивної розмови, а не ще один шар подій.
...
def main():
    ...
    def fit_one(key, cnt_tr, cnt_te, alphas, rival=None, kind_hint='theme'):
        ...
        # ЯКУ МОДЕЛЬ ПОКАЗУЄМО НА КАРТІ. Зміна 7 вересня.
        # Було: показували модель «тільки середовище» (pB) — вона свідомо НЕ
        # знає, де події вже сталися. Через це шар ризику майже не перетинався
        # з подіями, і це виглядало як помилка, хоча було задумом.
        # Це неправильно з двох причин:
        #   1) очікування нормальної людини — «небезпечні місця це передусім
        #      ті, де вже негаразд, ПЛЮС іще кілька, куди варто глянути
        #      наперед». Шар, який показує тільки друге, читається як збій;
        #   2) за числами модель «разом» майже завжди точніша: у 17 з 18
        #      шарів hit_разом > hit_середовище.
        # Стало: показуємо модель «разом» (pC = середовище + історія подій).
        # Щоб не втратити те, заради чого робили середовище, кожна вулиця
        # несе ознаку «чи були там події за період навчання»: на карті вони
        # малюються по-різному, і «тиха, але ризикована» вулиця лишається
        # видимою окремим класом, а не губиться серед уже гарячих.
        r = float(np.corrcoef(pC, yt)[0, 1]) if yt.std() > 0 else 0.0
        co = sorted(zip(fn, mB.coef_), key=lambda x: -abs(x[1]))
        rk = pC / (pC.max() or 1)

        # ---- КРАТНІСТЬ замість коефіцієнта (зміна 10 вересня) ----
        # «вага чинника +0.068» — це стандартизований коефіцієнт пуассонівської
        # регресії. Поза моделлю він не означає нічого: не відсоток, не разів,
        # не ймовірність. Risk Terrain Modeling (Caplan, Kennedy) практикам
        # коефіцієнтів не показує взагалі — лише порівняльне речення на кшталт
        # «очікувана частота втричі вища».
        #
        # Кратність рахується з ТИХ САМИХ даних простим діленням: беремо
        # відрізки, де ознаки більше за міську медіану, і ділимо щільність
        # подій на метр на ту, що на решті відрізків. Це число перевіряється
        # рахунком і не залежить від того, яку модель ми обрали.
        #
        # Поріг у 30 відрізків з кожного боку — щоб кратність не рахувалася
        # на випадковій жменьці. Там, де ознака є майже скрізь або майже
        # ніде, чесніше не називати числа зовсім.
        nb = len(base_n)
        med_all = np.median(X0, axis=0)
        def _ratio(j):
            v = X0[:, j]; hi = v > med_all[j]; lo = ~hi
            if hi.sum() < 30 or lo.sum() < 30: return None
            eh, el = expo[hi].sum(), expo[lo].sum()
            if eh <= 0 or el <= 0: return None
            rlo = y[lo].sum() / el
            if rlo <= 0: return None
            return round(float((y[hi].sum() / eh) / rlo), 2)
        ratios = {j: _ratio(j) for j in range(nb)}

        # ---- ЧИННИКИ КОЖНОЇ ВУЛИЦІ ОКРЕМО ----
        # Досі вікно вулиці показувало коефіцієнти ШАРУ: усі 200 вулиць
        # «Дорожнього руху» мали однаковий перелік чинників. Виглядало це як
        # поадресний розбір, а було переказом моделі — і лист, написаний із
        # такого вікна, не витримав би питання «з чого ви це взяли».
        #
        # Тепер для кожної вулиці рахуємо, які ознаки підняті САМЕ на ній:
        # внесок = коефіцієнт × відхилення ознаки від міського середнього
        # (для лінійної моделі це і є розклад оцінки на доданки). Поруч
        # зберігаємо саме число й міську медіану — щоб твердження можна було
        # перевірити, вийшовши на місце й порахувавши.
        #
        # Взаємодії сюди не беремо навмисно: «ринки_250м × школи_500м» людині
        # не пояснити, а перевірити на місці — то й поготів.
        contrib = Xs[:, :nb] * mB.coef_[:nb]
        def _street_facts(i, k=3):
            c = contrib[i]; out = []
            for j in np.argsort(-c)[:k]:
                if c[j] <= 0: break
                out.append([base_n[j], round(float(X0[i, j]), 1),
                            round(float(med_all[j]), 1), ratios.get(int(j))])
            return out
        ...
        ordr = np.argsort(-rk)
        def _row(i, facts=True):
            # `grid` — прихований хвіст на 200 вулиць, його ніхто не читає,
            # тож чинники туди не кладемо: це подвоїло б розмір файла дарма.
            row = [segs[sids[i]][::max(1, len(segs[sids[i]])//8)],
                   names[sids[i]], round(float(rk[i]), 3), int(y[i])]
            if facts: row.append(_street_facts(i))
            return row

        # довжина переліку — своя для кожного шару, за витримкою точності
        def _prec(n):
            sel = ordr[:n]
            return float(sum(1 for i in sel if yt[i] > 0)) / max(len(sel), 1)
        prec_curve, yield_curve, n_show = [], [], TOPN
        _prev = 0
        for n_ in NGRID:
            ...
            seg = ordr[_prev:n_]
            yield_curve.append([n_, round(float(yt[ordr[:n_]].sum()) / n_, 2),
                                round(float(yt[seg].sum()) / max(len(seg), 1), 2)])
            _prev = n_
            if n_ <= TOPN:                     # 25 показуємо завжди, як і раніше
                n_show = max(n_show, n_) if p_ >= PREC_MIN else n_show
                continue
            if p_ >= PREC_MIN: n_show = n_
            else: break                         # далі перелік уже розмиває знання

        # ---- «УМОВИ Є, ПОДІЙ ЩЕ НЕМАЄ» (зміна 10 вересня) ----
        # Доти ця думка жила у вигляді пунктирної лінії серед суцільних. На
        # теперішніх даних пунктиру в головних темах фактично немає: у
        # «Дорожньому русі» 0 вулиць з 200, у «Громадському порядку» 2 з 200.
        # Це прямий наслідок переходу на модель «разом» — вона й ставить
        # угору те, де події вже були. Тобто клас, заради якого будували шар
        # середовища, з карти зник, а панель про нього досі розповідала.
        #
        # Тепер це окремий перелік: верхні вулиці за моделлю ТІЛЬКИ
        # СЕРЕДОВИЩА серед тих, де за період навчання не було жодної події.
        # Саме з ним має сенс іти до балансоутримувача наперед.
        quiet = []
        if kind_hint == 'theme':
            oq = [i for i in np.argsort(-pB) if y[i] == 0][:QUIET_N]
            quiet = [_row(i) for i in oq]
        layer = {
            'hit': round(hC, 3),
            'hit_env': round(hB, 3),
            'items': [_row(i) for i in ordr[:n_show]],
            'quiet': quiet,
            'grid':  [_row(i, False) for i in ordr[:TOPGRID]]}
        ...
        entry = {
            ...
            'PAI_разом': round(hC / base, 2),
            'вулиць_показано': len(shown), 'з_них_збулося': збулося,
            # Скільки відрізків вулиць модель оцінювала — щоб карта могла
            # сказати «серед 3% вулиць міста», а не «місце 7 із 200»
            # (розд. 25, А8). З'являється з наступним перенавчанням.
            'відрізків': int(len(rk)),
            'точність_за_довжиною': prec_curve,
            # Скільки подій перевірочних років припадає на одну показану
            # вулицю — і скільки дає КОЖЕН наступний шматок переліку.
            # Поки що лише записуємо: частка вулиць із подіями для частих
            # тем не обмежує нічого (у «Дорожньому русі» вона тримається
            # 100% аж до 200 вулиць), а міняти довжину наосліп, не бачачи
            # цих чисел на справжньому прогоні, було б безвідповідально.
            'віддача_за_довжиною': yield_curve,
            'фактори': [[n, round(float(c), 3)] for n, c in co if abs(c) > 0.02][:20],
            # кратність і медіана — щоб документи й карта говорили «у N разів
            # більше», а не «вага +0.068». Ключ — база ознаки без радіуса.
            'кратність': {base_n[j]: ratios[j] for j in range(nb) if ratios[j]},
            'медіани': {base_n[j]: round(float(med_all[j]), 1) for j in range(nb)}}
    ...
    def method_text(e, wname):
        ...
        return (f"Модель навчена на {n1} подіях зі справ, розглянутих "
                ...
                f"Вона зважує і умови середовища, і те, що вже ставалося на "
                f"вулиці. У верхніх 10% вулиць за прогнозом опиняється "
                f"{100*e['hit_разом']:.0f}% подій наступних років "
                f"(у {uatext.raziv(e['PAI_разом'])} краще за випадковий відбір). "
                f"Сама лише обстановка, без історії подій, дає "
                f"{100*e['hit_середовище']:.0f}% — саме вона й підказує "
                f"вулиці, де подій ще не було, але умови ті самі."
                + tail)
    ...
    def show(title, e):
        ...
        print(f"      історія подій:   {100*e['hit_історія']:5.1f}%   (у {uatext.raziv(e['PAI_історія'])} краще за випадок)")
        print(f"      середовище:      {100*e['hit_середовище']:5.1f}%   (у {uatext.raziv(e['PAI_середовище'])})")
        ...
        if sh:
            print(f"   ПОКАЗУЄМО {sh} вулиць — події потім сталися на {ok} "
                  f"({100*ok/sh:.0f}%)")
            pc = e.get('точність_за_довжиною') or []
            if pc:
                print('      точність за довжиною переліку: '
                      + ', '.join(f'{n}:{100*p:.0f}%' for n, p in pc))
    ...
    for sim in all_sims:
        ...
        for w in range(len(WINDOWS)):
            got = fit_one(sim, TR[w][sim], TE[w][sim], ALPHA_M,
                          rival=theme_rival(w, th), kind_hint='механізм')
            if got: break
        if not got: continue
        e, lay, _p = got
        nm = M.simname(sim)
