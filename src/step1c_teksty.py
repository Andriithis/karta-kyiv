# -*- coding: utf-8 -*-
"""Крок 1c. Прохід по текстах рішень (ZAVDANNYA-TEKSTY.md, PLAN-TEKSTY.md).

З одного тексту рішення беремо все, що потрібно карті, за один раз:
фабулу з логічними межами, дату й час події з опису, речення, з якого взято
адресу, саму адресу за чинним addr.extract() і клас адреси B/C/D — на
повній фабулі, а не на обрізку 600 знаків, як у кроці 1b.

Тут поки лише розбір одного тексту. Черга, бюджет запуску й частини
data/teksty/<дата>.csv.gz — після того, як затвердять 20 прикладів фабул
(перша зупинка на перевірку, RISHENNYA, розд. 21).

Прохід (main): черга представників справ — МАЙ, НАР, НАС → ГП, АЛК, СЕР →
ДОР, усередині виду від найгустіших адрес; бюджет документів на запуск
(MAX_DOCS); результат — частина data/teksty/<дата>.csv.gz лише з
документами цього запуску. Наступний запуск продовжує з того самого місця:
зроблене — це все, що вже лежить у частинах з тим самим правилом RULE.

Запуск: py -3 src\\step1c_teksty.py          (MAX_DOCS=2000 — бюджет)
"""
import os, re, sys, csv, gzip, glob, time, datetime, threading, queue, collections
import urllib.request
sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
import addr as A
import labels as L
import podii as PD
import step1b_fabula as F
from step1_download import rtf_to_text, UA, WORKERS, DELAY

# ---- СТЕЛЯ ФАБУЛИ (затверджено 23.09) ----
# Виміряно на 140 текстах: логічна фабула — медіана 619 знаків, але вироки
# з кількома епізодами й описом майна тягнуть на кілька тисяч. Постанові
# КУпАП досить 1 500, вироку — 2 500. Ріжемо лише по межі речення.
CAP = {'КУпАП': 1000, 'КК': 2500}   # КУпАП 1 500 -> 1 000: на 240 текстах не спрацювала жодного разу


def kodeks(cat):
    """КК чи КУпАП — з підпису статті («ст.121 КК · …»)."""
    lb = (L.CODE.get(str(cat)) or ('', ''))[1]
    return 'КК' if re.search(r'\bКК\b', lb) else 'КУпАП'


# ---- ПОЧАТОК ФАБУЛИ ----
# У постановах КУпАП без «ВСТАНОВИВ» опис іде від «згідно (з) протоколу».
KUPAP_START = re.compile(r'згідно\s+(?:з\s+|із\s+)?протокол\w*', re.I)
# Сучасні вироки мають розділи: «І. Історія провадження» (процедура) і далі
# «Встановлені судом обставини» чи «Формулювання обвинувачення». Коли такий
# розділ є, фабула починається після його назви, а не після ВСТАНОВИВ —
# інакше вона відкривалася б абзацом про надходження обвинувального акта.
SECTION = re.compile(r'(?:обставини\s*,?\s*(?:що\s+|які\s+)?встановлені\s+судом|'
                     r'встановлені\s+судом\s+обставини|формулювання\s+обвинувачення'
                     r'(?:\s*,\s*визнаного\s+судом\s+доведеним)?)\s*[:.]?', re.I)
SECTION_WINDOW = 6000
# Сама назва розділу довга й сама по собі не фабула: «Формулювання
# обвинувачення, яке визнано судом доведеним (із зазначенням місця, часу,
# способу вчинення … мотивів кримінального правопорушення)». Її хвіст
# пропускаємо до крапки чи двокрапки після «доведеним» / «правопорушення».
SECTION_TAIL = re.compile(r'\s*[,(]?[^.:]{0,400}(?:доведеним|недоведеним|правопорушення|проступку)'
                          r'\s*\)?\s*[.:]?', re.I)
# Вирок, від якого проголошено лише вступну й резолютивну частини: опису
# події в ньому немає зовсім, повний текст виходить окремим документом.
SHORT_VERDICT = re.compile(r'(?:вступн\w*\s+(?:та|і)\s+резолютивн\w*\s+частин|'
                           r'проголошенням\s+його\s+резолютивн)', re.I)
# Речення на початку, які подією не є: надходження акта, реєстр, клопотання,
# і абзац про введення воєнного стану — з нього «датою події» ставала
# 24.02.2022. Відкидаємо їх, доки не почнеться опис (не далі восьми речень).
LEAD_JUNK = re.compile(r'надійш|обвинувальн\w*\s+акт|ЄРДР|Єдиного\s+реєстру|клопотан|'
                       r'в\s+порядку\s+ст|спрощен\w*\s+провадж|прокуратур|роз.?яснено|'
                       r'Указ\w*\s+Президента|воєнн\w+\s+стан|Закон\w*\s+України|'
                       # хвости абзацу про воєнний стан: «14.05.2024 строком на
                       # 90 діб тобто до 12.08.2024» — ні указу, ні учасника
                       r'строком\s+на\s+\d+\s+діб|продовж\w+\s+(?:строк|дію|з\s+05)|'
                       # вступ до фабули: «вчинив проступок за таких обставин»,
                       # «встановлено наступні обставини, які не оспорюються»
                       r'за\s+(?:таких|наступних)\s+(?:\S+\s+){0,4}?обставин|'
                       r'обставин\w*,?\s+які\s+не\s+оспорю|встановлено\s+наступн\w+\s+обставин', re.I)
# Опора: речення, де є і дата, і час, — це опис події, навіть якщо в ньому
# трапився реєстр чи воєнний стан. Таке речення ніколи не відкидаємо.
_D = r'(?:\d{1,2}[.\s/-]\d{1,2}[.\s/-]20\d{2}|\d{1,2}\s+(?:січня|лютого|березня|квітня|травня|червня|' \
     r'липня|серпня|вересня|жовтня|листопада|грудня)\s+20\d{2})'
_T = r'(?:(?:\bо|\bоб|близько|приблизно|орієнтовно)\s*\d{1,2}\s*(?:год|:|\.\d\d)|\bз\s*\d{1,2}[.:]\d{2})'
HAS_DATE, HAS_TIME = re.compile(_D, re.I), re.compile(_T, re.I)


# Абзац про введення воєнного стану теж має дату й час («з 05 год. 30 хв.
# 24 лютого 2022 року»), але подією не є, поки в ньому немає учасника.
MARTIAL = re.compile(r'Указ\w*|воєнн\w+\s+стан\w*|Закон\w*\s+України', re.I)
ACTOR = re.compile(r'ОСОБА_\d|Особа\s+1')


# Дата й час засідання — теж не подія: «у судове засідання, призначене на
# 30.09.2025 о 10:00, не з'явилась». З таких постанов (про привід, про
# об'єднання справ) опису правопорушення не буває.
# Привід — лише як процесуальний захід: «з приводу того, що…» у фабулі —
# звичайний зворот, і він робив шкідливою цілком нормальну фабулу.
COURT_PROC = re.compile(r'судов\w+\s+засідан|призначен\w+\s+на\s+\d|не\s+з.?явил|'
                        r'(?:піддати|піддано|виконання|застосувати|застосування)\s+привод\w*|привід\s+ОСОБА|'
                        r'об.?єдна\w+\s+(?:справ|провадж)|провадженні\s+суду\s+перебува|'
                        r'звернув\w*\s+до\s+суду|речов\w+\s+доказ|справ\w*\s+.{0,60}розглянут', re.I)


# Адреса поруч із датою — теж ознака опису, коли часу в протоколі не вказано.
ADDR_CUE = re.compile(r'за\s+адрес\w*|по\s+(?:вул|просп|бульв|пров|пл)\w*\.?|біля\s+буд|АДРЕСА_\d', re.I)


def is_event_sentence(s):
    d = HAS_DATE.search(s)
    if not d or (MARTIAL.search(s) and not ACTOR.search(s)):
        return False
    # процедура вбиває речення, лише коли стоїть ПЕРЕД датою: «у судове
    # засідання, призначене на 30.09.2025 о 10:00». Після дати події вона —
    # уже продовження довгого речення, а не його суть.
    p = COURT_PROC.search(s)
    if p and p.start() < d.start():
        return False
    return bool(HAS_TIME.search(s) or ADDR_CUE.search(s, d.start()))


# «До суду надійшов протокол …. Відповідно до вказаного протоколу, ОСОБА_1
# 22 серпня …» бувають одним реченням — опис починається з другої половини.
INNER_START = re.compile(r'(?:відповідно\s+до|згідно\s+(?:з\s+|із\s+)?|як\s+вбачається\s+з)\s*'
                         r'(?:вказаного\s+|даного\s+)?протокол', re.I)
WHICH_SAYS = re.compile(r'(?:згідно\s+(?:з|із)\s+яким|відповідно\s+до\s+якого|'
                        r'з\s+якого\s+(?:вбачається|слідує)\s*,?\s*що)\s*,?\s*', re.I)

# ---- КІНЕЦЬ ФАБУЛИ ----
# Поверх чинних STOP і TAIL кроку 1b: маркери з завдання й ті, що знайшлися
# на вибірці 23.09. Постанови КУпАП після фабули майже завжди переходять до
# шаблонних абзаців про ст. 7 і 268 КУпАП, вироки — до позиції обвинуваченого.
END = re.compile(r'(таким\s+чином|'
                 r'(?:в|у)\s+судовому\s+засіданні|'
                 r'вин[уи]\s+(?:у\s+\S+\s+)?(?:не\s+)?визна(?:в|ла|ли|є)\w*|'
                 r'дослідивши\s+матеріали|'
                 r'визна[єю]ть?ся\s+винн?уват\w*|визнається\s+винн\w+|'
                 r'кваліфікова\w+|кваліфіку\w+|\b[IVXІ]+\.\s*(?:Статт|Доказ)\w*|'
                 r'тобто\s+(?:вчинив|вчинила|скоїв|скоїла)\w*\s+(?:адміністративн|кримінальн)|'
                 r'за\s+викликом\s+до\s+суду|'
                 # косметичні хвости постанов (рішення 23.09): «Як вважає особа,
                 # яка склала протокол…», «про місце, день і час розгляду справи
                 # повідомлявся…»
                 r'як\s+вважає\s+особа|(?:про|щодо)\s+(?:місце|дату|день|час)\w*[,\s\w]{0,40}?розгляду|'
                 r'згідно\s+(?:зі|з|із|до)\s+(?:списк|Перелік|таблиц)\w*|постановою\s+Кабінету\s+Міністрів|'
                 r'що\s+підтверджено\s+висновк|'
                 r'кримінальна\s+відповідальність\s+за|санкці\w+\s+ч|щодо\s+призначення|'
                 r'доводиться\s+(?:висновком|матеріалами|доказами)|доводять\w*|'
                 r'подія\s+(?:та|і)\s+склад|згідно\s+(?:з\s+|із\s+)?висновк|'
                 r'\d\.\s*Статт\w+\s*\(частин|'
                 r'належним\s+чином\s+повідомлен|'
                 r'згод\w+\s+з\s+розглядом|не\s+оспорю|ураховуючи\s+наведене|беззаперечно\s+визна|'
                 r'своїми\s+(?:\S+\s+){0,2}?діями|за\s+що\s+передбачен|'
                 r'визна\w*\s+(?:свою\s+)?вин|не\s+заперечува|'
                 r'(?:у|в)\s+відповідності\s+(?:з|із|до)\s+п|'
                 r'ст\.\s*(?:245|251|252|268|277|280)\s+КУпАП|при\s+розгляді\s+справи|'
                 r'ЄСПЛ|Європейськ\w+\s+суд|повторно\s+надійш|'
                 r'(?:в|у)\s+судове\s+засідання|не\s+з.?явил|'
                 r'\bКПК\b|'
                 r'згідно\s+з\s+ч\.\s*\d+\s*ст\.\s*7\s+КУпАП|'
                 r'відповідно\s+до\s+ст\.\s*268|'
                 r'з\s+аналізу\s+ст)', re.I)
MIN_BEFORE_END = 60        # маркер кінця на самому початку — не кінець, а зворот
# Після маркера кінця від останнього речення часто лишається уривок: «У
# результаті протиправних дій ОСОБА_5». Коротший за це — відкидаємо до межі
# речення: обірване речення гірше, ніж на одне речення коротша фабула.
STUB = 90

MONTHS = {m: i + 1 for i, m in enumerate(
    'січня лютого березня квітня травня червня липня серпня вересня жовтня листопада грудня'.split())}
# (?<!\d), а не \b: «встановлено, що23.03.2024» — пробіл загубився, і \b між
# літерою й цифрою не спрацьовує
DATE_NUM = re.compile(r'(?<!\d)(\d{1,2})[.\s/-](\d{1,2})[.\s/-](20\d{2})')
DATE_TXT = re.compile(r'(?<!\d)(\d{1,2})\s+(' + '|'.join(MONTHS) + r')\s+(20\d{2})', re.I)
# «приблизно об 11 год.30 хв.» (Героїв Космосу, 1) часу не давало: після
# «приблизно» чекали одразу цифру. «о» — лише окремим словом, не кінцем «до».
TIME = re.compile(r'(?:\bоб?|близько|приблизно|орієнтовно|біля)(?:\s+об?)?\s*(\d{1,2})\s*(?:год\w*\.?|:)\s*(\d{2})?', re.I)


# Назва розділу стоїть з нового рядка, іноді з номером («2.», «ІІ.»). Ті
# самі слова всередині абзацу мотивів («за обставин, встановлених судом»)
# розділом не є — інакше фабула починалася б із мотивів.
HEADING_BEFORE = re.compile(r'(?:^|[\n\r])[ \t]*(?:[IVXІ]+\.?|\d+(?:\.\d+)*\.?)?[ \t]*$')


def _section(text, bs):
    for m in SECTION.finditer(text, bs, bs + SECTION_WINDOW):
        if HEADING_BEFORE.search(text[max(0, m.start() - 12):m.start()]):
            return m
    return None


# Межі, яких крок 1b не бачить: речення, що починається з дати («… не
# оспорюються. 16.01.2026 року приблизно о 23:00 …»), і речення після суми
# («… 49 448 грн. 80 коп. Будучи …»). Без першої процедура й подія злипалися
# в одне «речення», і відкинути процедуру було неможливо.
# Третя — «ОСОБА_3 . Під час…»: реєстр знеособлює з пробілом перед крапкою.
EXTRA_BOUND = re.compile(r'[А-ЯІЇЄҐа-яіїєґ»)]\.\s+(?=' + _D + r')|\b(?:грн|коп)\.\s+(?=[А-ЯІЇЄҐ][а-яіїєґ])|'
                         r'(?<=\w)\s\.\s+(?=[А-ЯІЇЄҐ])')


def _bounds(t):
    """Межі речень: кроку 1b плюс EXTRA_BOUND, але «04.06.2023р.» речення
    не кінчає."""
    b = set(F._bounds(t))
    for m in EXTRA_BOUND.finditer(t):
        b.add(m.start() + m.group(0).index('.') + 1)
    # лише «2023р.»: «…до 15 листопада 2023. ОСОБА_3…» — справжня межа.
    # «пр-т.», «б-р.» — скорочення типу вулиці, крок 1b їх не знає: «за
    # адресою: м. Київ, пр-т. Степана Бандери» рвалося на «пр-т.»
    return sorted(x for x in b if not re.search(r'\d{4}\s?р\.$|\b(?:пр-т|б-р|пр|вул|просп|бульв)\.$',
                                                t[:x].rstrip()))


def _start(text):
    bs = A.body_start(text)
    sec = _section(text, bs)
    if sec:
        tail = SECTION_TAIL.match(text, sec.end())
        return tail.end() if tail else sec.end()
    if bs:
        return bs
    m = KUPAP_START.search(text)
    return m.start() if m else 0


# «Як встановлено в судовому засіданні, ОСОБА_2, 19.12.2024…» — вступний
# зворот; без нього «судовому засіданні» вбивало б речення з самою подією.
OPEN_COURT = re.compile(r'^(?:як\s+)?(?:судом\s+)?(?:встановлено|встановив\s+суд)\s+(?:в|у)\s+судовому\s+'
                        r'засіданні\s*,?\s*(?:що\s*,?\s*)?', re.I)
# Процедура, яка лишається процедурою навіть з учасником і датою в реченні.
HARD_JUNK = re.compile(r'надійш|ЄРДР|Єдиного\s+реєстру|клопотан|обвинувальн\w*\s+акт', re.I)


def _is_junk(s):
    """Процедурне речення. Учасник із датою — уже опис («ОСОБА_3 20 серпня
    2023 року, діючи в умовах воєнного стану, вирішив…»), якщо тільки це не
    надходження справи чи реєстр."""
    if COURT_PROC.search(s):
        return True             # засідання, неявка, привід — процедура завжди
    if not LEAD_JUNK.search(s):
        return False
    if ACTOR.search(s) and HAS_DATE.search(s) and not HARD_JUNK.search(s):
        return False
    return True


def _drop_lead(t):
    """Відкидає процедурні речення й абзац про воєнний стан на початку.
    Абзац про воєнний стан з переліком усіх продовжень буває на десяток
    речень, тож межа — 15."""
    for _ in range(15):
        b = _bounds(t)
        first = t[:b[0]] if b else t
        junk = _is_junk(first)
        if is_event_sentence(first) or not junk or not b:
            if junk and re.search(r'надійш', first):
                m = INNER_START.search(first, 20)
                if m:
                    t = t[m.start():]
                else:
                    # «…надійшов протокол …, згідно з яким ОСОБА_1, 28.01.2025…»
                    m = WHICH_SAYS.search(first)
                    if m:
                        t = t[m.end():]
            break
        t = t[b[0]:].lstrip()
    return t


def _event_anchor(t):
    """Перше речення з датою й часом — звідти починається опис, коли звичайні
    маркери привели не туди (позиція сторін, роз'яснення прав, шапка)."""
    starts = [0] + _bounds(t)
    for i, s in enumerate(starts):
        e = starts[i + 1] if i + 1 < len(starts) else len(t)
        if is_event_sentence(t[s:e]):
            return s
    return None


def _cut(t, cap):
    """Не довше за cap, і лише по межі речення. Якщо межі немає зовсім —
    по слову з трикрапкою: так видно, що текст неповний."""
    if len(t) <= cap:
        return t
    b = [x for x in _bounds(t) if x <= cap]
    return t[:b[-1]] if b else t[:cap].rsplit(' ', 1)[0] + '…'


# Хвіст назви розділу вироку, що лишається на початку фабули: «(місце, час,
# спосіб вчинення та наслідки…) з урахуванням зміненого обвинувачення», і
# нумерація пунктів «1.1.» (рішення 24.09, п.4).
LEAD_PAREN = re.compile(r'^[\s,]*\([^)]{0,300}?наслідк[^)]{0,200}\)\s*[,.:]?\s*'
                        r'(?:з\s+урахуванням\s+зміненого\s+обвинувачення\s*[,.:]?\s*)?', re.I)
LEAD_CHANGED = re.compile(r'^[\s,]*з\s+урахуванням\s+зміненого\s+обвинувачення\s*[,.:]?\s*', re.I)
# Лише «1.1. », «2.3.1. » — з крапкою в кінці: дата «14.08.2025 » на
# початку фабули під це не підпадає (перша версія шаблону її зрізала).
LEAD_NUM = re.compile(r'^\d{1,2}(?:\.\d{1,2}){1,3}\.\s+')


def tidy_start(t):
    t = LEAD_PAREN.sub('', t)
    t = LEAD_CHANGED.sub('', t)
    return LEAD_NUM.sub('', t).lstrip(' ,.:;')


def fabula(text):
    """Фабула з логічними межами, без стелі: (текст, чи знайдено кінець)."""
    # «\~», «\*» — залишки RTF, які rtf_to_text не прибрав
    body = re.sub(r'\s+', ' ', re.sub(r'\\[~*\-]', ' ', text[_start(text):])).strip()
    if SHORT_VERDICT.search(body[:400]):
        return '', False
    t = F.OPEN.sub('', body).strip()
    t = OPEN_COURT.sub('', t).strip()
    t = tidy_start(t)
    t = F._drop_preamble(t)
    t = _drop_lead(t)
    fab, found = _ends(t)
    # Фабула без дати або з одного звороту — маркери привели не туди. Тоді
    # беремо від першого речення з датою й часом у тексті після шапки.
    # Коротка постанова КУпАП з датою — нормальна, її не чіпаємо.
    if len(fab) < 120 or not event_date(fab):
        a = _event_anchor(body)
        if a is not None:
            fab2, found2 = _ends(body[a:])
            if len(fab2) >= len(fab) or not event_date(fab):
                return fab2, found2
        elif not event_date(fab):
            # Ні дати події, ні речення з датою й часом — це не фабула, а
            # процедура: постанова про привід, про об'єднання справ, абзац про
            # права сторін. Справжня фабула дату має майже завжди (138 зі 140
            # на вибірці 23.09). Чесне «опису немає» (клас D) краще за уривок.
            return '', False
    # Кожне речення — процедура (клопотання про речовий доказ, стара постанова
    # у справі): опису немає, хоч дата й трапилась.
    st = [0] + _bounds(fab)
    sents = [fab[a:b] for a, b in zip(st, st[1:] + [len(fab)]) if fab[a:b].strip()]
    if sents and all(_is_junk(s) for s in sents) and not any(is_event_sentence(s) for s in sents):
        return '', False
    return fab, found


# «будучи притягнутим … за дії, передбачені ч. 1 ст. 130 КУпАП, керував …»:
# тут «передбачені» — про попереднє стягнення, а фабула йде далі.
# Так само «від невстановленої особи, матеріали щодо якої виділено, за
# ознаками правопорушення, передбаченого ч. 1 ст. 307 КК, придбала …» —
# це стаття іншої особи посеред опису.
PRIOR = re.compile(r'(?:за\s+(?:дії|правопорушення)\s*,?|притягнут\w+.{0,80}|за\s+ознаками\s+.{0,70})\s*$', re.I)


def _stop(t):
    for m in F.STOP.finditer(t):
        g = m.group(0).lower()
        if g.startswith('передбачен') and PRIOR.search(t[max(0, m.start() - 120):m.start()]):
            continue
        # «знайшла, тим самим незаконно придбала …. В подальшому, 14.02.2024
        # о 13.30 … було зупинено» — у наркотичних справах «тим самим
        # придбав» стоїть посеред опису, коли далі ще є епізод із датою й часом
        if g.startswith('тим') and _event_anchor(t[m.end():m.end() + 1500]) is not None:
            continue
        return m.start()
    return None


# «…без марок акцизного податку, у зв'язку з чим його дії», «…без мети
# збуту, тому його дії», «…тобто вчинив правопорушення», «…за що
# відповідальність» — від кваліфікації лишився зв'язковий зворот без
# присудка. На частині v2 таких хвостів — 349 із 30 000.
# «внаслідок чого автомобілі отримали механічні пошкодження» — наслідок
# події, його не чіпаємо: сполучник відрізається, лише коли після нього
# лишився уривок (до 20 знаків). Кваліфікація — повністю.
TAIL_LINK = re.compile(r'[,\s]+(?:(?:у\s+зв.?язку\s+з\s+чим|тим\s+самим|внаслідок\s+чого|чим)\b[^.,;]{0,20}|'
                       r'(?:тому|отже)\s+(?:його|її|їх)\s+дії\b[^.,;]{0,40}|'
                       r'тобто\b[^.,;]{0,60}|за\s+що\b[^.,;]{0,60})$', re.I)
# «…Відповідальність за вказане адміністративне правопорушення передбачена
# ст.124 КУпАП» — окреме речення кваліфікації в кінці постанови (Хрещатик, 25,
# ДТП 26.02.2025). На частині v2 — 64 фабули. «Відповідальність за незаконний
# обіг…» всередині опису не чіпаємо: відрізаємо лише речення про «вказане».
TAIL_VIDP = re.compile(r'[\s.,;]*\bВідповідальність\s+за\s+вказан\w*\s+(?:адміністративн\w*\s+)?'
                       r'правопорушен\w*,?\s+передбачен\w*\b.{0,160}$', re.I | re.S)


def tidy_end(t):
    """Відрізає зв'язковий хвіст кваліфікації. Карта застосовує це й до вже
    записаних частин, тож дані для цього переписувати не треба."""
    if not t:
        return t
    new = TAIL_LINK.sub('', TAIL_VIDP.sub('', t))
    return new.rstrip(' ,.;:—-(') if new != t else t


def _ends(t):
    e = END.search(t, MIN_BEFORE_END)
    ends = [x for x in (_stop(t), e.start() if e else None) if x is not None]
    m2 = F.TAIL.search(t)
    if m2 and m2.start() >= 80:
        ends.append(m2.start())
    found = bool(ends)
    if ends:
        t = t[:min(ends)]
        b = _bounds(t)
        if b and len(t) - b[-1] < STUB:
            t = t[:b[-1]]
    t = t.rstrip(' ,.;:—-(')
    # «…крововилив в кон'юнктиві лівого ока, які [згідно висновку]» — маркер
    # відрізав підрядне речення, від нього лишилося сполучне слово
    t = re.sub(r'[,\s]+(?:які|який|яка|яке|що|та|і|й|а|де)$', '', t)
    # «…мобільний телефон «Самсунг», вартість якого [згідно висновку]» —
    # від підрядного лишився початок без присудка
    t = re.sub(r',[^,]{0,40}\b(?:якого|якої|яких|якому|якій)$', '', t)
    t = tidy_end(t)
    # «…де останнього було затримано, згідно ст. 298-2» — від посилання на
    # статтю лишився самий номер
    t = re.sub(r'[,\s]+(?:згідно|відповідно\s+до|на\s+підставі|в\s+порядку)\s+(?:з\s+)?(?:ч\.?\s*\d+\s*)?'
               r'ст\.?\s*[\d\-]+$', '', t)
    return t, found


# Дата поруч із цими словами — дата указу, закону, реєстрації чи
# надходження справи, а не події.
# Вікно коротке (DATE_CTX): «протоколу серії ГП №532688 від 17 лютого 2024
# року, ОСОБА_1 17.02.2024 о 01:00» — номер протоколу стоїть перед датою
# протоколу, а не перед датою події.
DATE_NOT_EVENT = re.compile(r'Указ|Закон|воєнн|ЄРДР|реєстр|надійш|засідан|призначен\w*\s+на|від\s*$', re.I)
DATE_CTX = 30
# Речення, дата з якого не буває датою події (рішення 24.09, п.3): наказ
# про призначення на посаду, ухвала, реєстрація провадження, попередня
# судимість, довідка, СІЗО. На першій частині проходу 833 події дістали
# дату до 2022 року при рішенні 2024+, і частина з них — саме звідси.
DATE_SENT_NOT_EVENT = re.compile(r'наказ|ухвал|провадженн\w*\s*№|судим|засуджен|довідк|СІЗО|'
                                 r'ізолятор|ув.?язнен', re.I)


def event_date(fab):
    """Перша дата в описі, що не є датою документа, — дата події. ISO або ''."""
    cands = []
    for m in DATE_NUM.finditer(fab):
        d, mo, y = int(m.group(1)), int(m.group(2)), int(m.group(3))
        if 1 <= d <= 31 and 1 <= mo <= 12:
            cands.append((m.start(), f'{y:04d}-{mo:02d}-{d:02d}'))
    for m in DATE_TXT.finditer(fab):
        cands.append((m.start(), f'{int(m.group(3)):04d}-{MONTHS[m.group(2).lower()]:02d}-{int(m.group(1)):02d}'))
    bounds = [0] + _bounds(fab) + [len(fab)]
    for pos, iso in sorted(cands):
        if DATE_NOT_EVENT.search(fab[max(0, pos - DATE_CTX):pos]):
            continue
        a = max(b for b in bounds if b <= pos)
        z = min((b for b in bounds if b > pos), default=len(fab))
        # «ОСОБА_3, будучи раніше судимим, 21.11.2024 о 14 год … за адресою» —
        # саме речення події: судимість у ньому лише згадана
        if DATE_SENT_NOT_EVENT.search(fab[a:z]) and not is_event_sentence(fab[a:z]):
            continue
        # «з 05 години 30 хвилин 24 лютого 2022 року», «продовжено з 05
        # листопада 2025» — дати указів про воєнний стан, а не подія, навіть
        # коли речення далі переходить до учасника. Дата належить указу, якщо
        # між згадкою указу й датою ще не було учасника.
        before = fab[max(0, pos - 300):pos]
        # лише в межах того самого речення: указ у попередньому реченні дату
        # події вже не зачіпає
        sb = list(re.finditer(r'\.\s+(?=[А-ЯІЇЄҐ\d])', before))
        if sb:
            before = before[sb[-1].end():]
        after = fab[pos:pos + 150]
        mm = list(MARTIAL.finditer(before))
        if iso == '2022-02-24' or (mm and not ACTOR.search(before[mm[-1].end():])):
            if mm or MARTIAL.search(after):
                continue
        # «14.05.2024 строком на 90 діб» — продовження воєнного стану;
        # «засуджений з 07.04.2021 відбував покарання», «судимим, а саме 30
        # травня 2023» — дата старого вироку, а не цієї події
        if re.match(r'[^.]{0,30}?строком\s+на\s+\d+\s+діб', after) or \
                PRIOR_CASE.search(before[-150:]) or re.search(r'відбува\w+\s+покаранн', after[:80]):
            continue
        return iso
    return ''


def event_time(fab):
    for m in TIME.finditer(fab):
        hh, mm = int(m.group(1)), int(m.group(2) or 0)
        if 0 <= hh <= 23 and 0 <= mm <= 59:
            return f'{hh:02d}:{mm:02d}'
    return ''


# ---- ПОЗНАЧКА ПЕРЕВІРКИ (рішення 23.09 до ZVIT-TEKSTY-1.md) ----
# Внутрішня, у даних; в інтерфейсі жодних написів. «шкідлива» — фабула на
# сайт не йде (панель показує рішення без опису), клас адреси — D.
# «косметична» — фабула на сайті, бо подія в ній є, а вада — у рамці.
# Прийменник чи сполучник останнім словом — обрив. Лише після пробілу:
# «д.н.з» (номерний знак) закінчується на «з», але це скорочення.
HARMFUL_END = re.compile(r'(?:^|\s)(?:у|в|з|із|зі|до|на|по|від|за|про|під|над|при|через|біля|та|і|й|а|але|'
                         r'що|чим|який|яка|яке|які|якого|якої|зв.?язку|тобто|оскільки|бо)$', re.I)
PRIOR_CASE = re.compile(r'судим|засуджен|вироком\s+\w+\s+(?:районного|суду)', re.I)
LEGAL_TAIL = re.compile(r'порушив\w*\s+(?:вимог\w*\s+)?п(?:ункт\w*)?\.|\bПДР\b|що\s+тягне|'
                        r'передбачен\w+\s+(?:ч\.|ст)|Правил\w*\s+дорожнього', re.I)


def _sentences(t):
    st = [0] + _bounds(t)
    return [t[a:b] for a, b in zip(st, st[1:] + [len(t)]) if t[a:b].strip()]


def vada(fab, found):
    """'' | 'косметична' | 'шкідлива' — ZAVDANNYA-TEKSTY.md, рішення 23.09."""
    if not fab:
        return ''                  # опису немає — це клас D, а не вада витягу
    sents = _sentences(fab)
    # не та частина тексту: жодного речення з подією, а починається з указу
    # без учасника чи з попередньої судимості
    first = sents[0]
    wrong_part = not any(is_event_sentence(s) for s in sents) and (
        (MARTIAL.search(first) and not ACTOR.search(first)) or PRIOR_CASE.search(fab[:400]))
    if (not found or HARMFUL_END.search(fab.rstrip(' ,.;:')) or COURT_PROC.search(fab)
            or wrong_part):
        return 'шкідлива'
    # хвіст — лише кінець фабули: «Правил дорожнього руху» всередині опису
    # ДТП — частина події, а не юридична рамка
    if _is_junk(first) or LEGAL_TAIL.search(fab[-160:]):
        return 'косметична'
    return ''


def mend(fab, found):
    """(фабула, позначка). Обрізаний кінець («…без мети збуту. PVP (1-феніл-
    …), згідно з») відрізаємо до останнього повного речення. Якщо лишилася
    фабула з датою й подією — вада в рамці, «косметична»; ні — «шкідлива»
    (рішення 24.09, п.2). У НАР так хибно шкідливими ставали 5,2% фабул:
    довідка про речовину після опису, обірвана стелею чи маркером."""
    fab = tidy_end(tidy_start(fab))
    trimmed = False
    if fab and HARMFUL_END.search(fab.rstrip(' ,.;:')):
        b = _bounds(fab)
        rest = fab[:b[-1]].rstrip(' ,.;:') if b else ''
        if rest and event_date(rest) and any(is_event_sentence(s) for s in _sentences(rest)):
            fab, trimmed = rest, True
    v = vada(fab, found)
    return fab, ('косметична' if trimmed and not v else v)


# Місце події в наркотичних справах (рішення 24.09, п.4) — де придбав,
# забрав закладку, зберігав чи де затримали, а не звідки замовляв. Фабула
# зазвичай починається з дому: «перебуваючи за адресою: АДРЕСА_1, через
# месенджер «Телеграм» домовився…», і за правилом «перша згадка» місцем
# ставала квартира (клас D) або будинок, звідки писали в месенджер.
NAR_ORDER = re.compile(r'\b(?:замовив\w*|замовил\w*|домовив\w*|домовил\w*|сплатив\w*|сплатил\w*|'
                       r'перерахував\w*|перерахувал\w*|месенджер\w*|інтернет\w*|телеграм\w*|telegram)', re.I)
NAR_PICK = re.compile(r'\b(?:підібра\w*|підбор\w*|забра\w*|знайш\w*|знахідк\w*|виявлен\w*|вилучен\w*|'
                      r'затрима\w*|зупин\w*)', re.I)


def nar_keep(text, ms):
    """Відсів для A.extract: згадка, у частині речення якої (від сусідньої
    згадки до сусідньої, в межах речення) є «замовив», «месенджер»… і немає
    «забрав», «затримано»…, — місце замовлення. Воно не дає місця події,
    якщо далі у фабулі є згадка з «забрав», «виявлено», «затримано».
    Якщо такої немає — лишається те, що є."""
    kind = []
    for i, (p, *_r) in enumerate(ms):
        lo = max(A.sentence_start(text, p), ms[i - 1][0] if i else 0)
        hi = min(A.sentence_end(text, p), ms[i + 1][0] if i + 1 < len(ms) else len(text))
        seg = text[lo:hi]
        kind.append('pick' if NAR_PICK.search(seg) else 'order' if NAR_ORDER.search(seg) else '')
    out = [m for i, m in enumerate(ms)
           if not (kind[i] == 'order' and 'pick' in kind[i + 1:])]
    return out or ms


def adresa(fab, text='', nar=False):
    """Адреса — спершу з самої фабули: у повному тексті body_start ловить
    пізнє «встановив» (у мотивах), і адреса з опису лишалася за межею
    пошуку — на першій частині проходу так без місця лишилося 172 фабули.
    Повний текст — лише запасний шлях, коли у фабулі місця не названо."""
    keep = nar_keep if nar else None
    t = 'ВСТАНОВИВ: ' + fab if fab else ''
    res = A.extract(t, keep) if t else dict(street=None, house=None, level='none', time=None, pos=None)
    if res['level'] == 'none' and text:
        res2 = A.extract(text, keep)
        if res2['level'] != 'none':
            res, t = res2, text
    sent = ''
    if res.get('pos') is not None:
        p = res['pos']
        sent = re.sub(r'\s+', ' ', t[A.sentence_start(t, p):A.sentence_end(t, p)]).strip()
    return res, sent


def rozbir(text, cat):
    """Усе, що прохід бере з одного тексту рішення."""
    full, found = fabula(text)
    full, v = mend(full, found)
    res, sent = adresa(full, text, nar=PD.theme(cat) == 'НАР')
    kd = kodeks(cat)
    return dict(
        fab=_cut(full, CAP[kd]), fab_len=len(full), end_found=found, kodeks=kd, vada=v,
        date=event_date(full), time=event_time(full) or (res.get('time') or ''),
        addr_sentence=sent, street=res['street'] or '', house=res['house'] or '',
        level=res['level'],
        # клас — на логічній фабулі до стелі: так межа B/C не залежить від
        # обрізу. Шкідлива фабула підтвердити адресу не може — D.
        klass='D' if v == 'шкідлива' else PD.addr_class(full, res['street'], res['level']))


def pererakhuvaty(r, cat=''):
    """Рядок частини зі старим правилом -> рядок за чинним, без повторного
    завантаження: фабула збережена, адреса й дата стоять у ній. Якщо у
    збереженій (обрізаній стелею) фабулі місця не названо, а старий розбір
    повного тексту його знайшов — лишаємо старе."""
    fab, found = r['fab'], r['end_found'] == '1'
    fab, v = mend(r['fab'], found)
    # збережену фабулу вже підрізано — повторний mend обрізу не бачить і
    # знімав би «косметичну» з 225 фабул, у яких нічого не змінилось
    if not v and fab == r['fab'] and r['vada'] == 'косметична':
        v = 'косметична'
    res, sent = adresa(fab, nar=PD.theme(cat) == 'НАР')
    if res['level'] == 'none' and r['level'] != 'none':
        res = dict(street=r['street'] or None, house=r['house'] or None, level=r['level'], time=None)
        sent = r['addr_sentence']
    out = dict(r)
    out.update(rule=RULE, vada=v, fab=fab, date=event_date(fab),
               time=event_time(fab) or r['time'], street=res['street'] or '', house=res['house'] or '',
               level=res['level'], addr_sentence=sent,
               klass='D' if v == 'шкідлива' else PD.addr_class(fab, res['street'], res['level']))
    return out


# ============================================================ ПРОХІД
ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
DATA = os.path.join(ROOT, 'data')
TEKSTY = os.path.join(DATA, 'teksty')
REESTR = 'https://od.reyestr.court.gov.ua/files/'
# Версія правил розбору. Частини зі старою версією в зроблене не рахуються —
# зміна правил означає новий прохід, а старі частини лишаються як історія.
# v2 (24.09): адреса з фабули, «пр.», вікно NOISE в межах речення, обрізаний
# кінець — косметична, дата не з речення про наказ, ухвалу, судимість.
# v3 (24.09): у наркотичних місце замовлення не місце події; хвіст
# «Відповідальність за вказане правопорушення передбачена ст. …».
RULE = 'v3'
ORDER = ['МАЙ', 'НАР', 'НАС', 'ГП', 'АЛК', 'СЕР', 'ДОР']
COLS = ['doc_id', 'rule', 'vada', 'klass', 'date', 'time', 'street', 'house', 'level',
        'addr_sentence', 'fab_len', 'end_found', 'fab']
SAVE_EVERY = 1000          # частину переписуємо по ходу: обрив запуску не губить зроблене


def load_done():
    """doc_id -> рядок останньої частини з поточним правилом (пізніша перемагає)."""
    done = {}
    for fp in sorted(glob.glob(os.path.join(TEKSTY, '*.csv.gz'))):
        with gzip.open(fp, 'rt', encoding='utf-8', newline='') as fh:
            for r in csv.DictReader(fh, delimiter='\t'):
                if r.get('rule') == RULE:
                    done[r['doc_id']] = r
    return done


def queue_docs():
    """Представники справ (вирок, постанова) з посиланням на текст, у порядку
    черги. Повертає (список записів events, doc_id -> посилання)."""
    with gzip.open(os.path.join(DATA, 'events.csv.gz'), 'rt', encoding='utf-8', newline='') as fh:
        rows = [r for r in csv.DictReader(fh, delimiter='\t') if r.get('doc_id', '').isdigit()]
    formy = PD.load_formy()
    links = PD.load_links()
    for fp in glob.glob(os.path.join(DATA, 'kyiv_*.csv')):
        with open(fp, encoding='utf-8-sig') as fh:
            for r in csv.DictReader(fh, delimiter='\t'):
                links[r['doc_id']] = (r['cause_num'], PD.docref(r['doc_url']))
    cause = {d: c for d, (c, _r) in links.items() if c}
    reps = [rep for rep, *_ in PD.merge_cases(
        rows, doc=lambda r: r['doc_id'], cat=lambda r: r['cat'], date=lambda r: r['date'],
        cause=cause, event=lambda r: formy.get(r['doc_id']) in PD.EVENT_FORMS)]
    # усередині виду — від найгустіших адрес: так проблемні місця
    # з'являються першими, а не наприкінці тижня
    dens = collections.Counter((r['street'] or '') + '|' + (r['house'] or '') for r in reps)
    rank = {t: i for i, t in enumerate(ORDER)}
    reps.sort(key=lambda r: (rank.get(PD.theme(r['cat']), 99),
                             -dens[(r['street'] or '') + '|' + (r['house'] or '')], r['doc_id']))
    return reps, {d: v[1] for d, v in links.items() if v[1]}


def _write(fp, out):
    """Частина: відсортовано, без часу в gzip — однаковий вміст дає однаковий файл."""
    os.makedirs(TEKSTY, exist_ok=True)
    with open(fp + '.tmp', 'wb') as raw, \
         gzip.GzipFile(filename='', mode='wb', fileobj=raw, mtime=0) as gz:
        w = csv.writer(_Utf8(gz), delimiter='\t', lineterminator='\n')
        w.writerow(COLS)
        for d in sorted(out, key=int):
            w.writerow([out[d].get(c, '') for c in COLS])
    os.replace(fp + '.tmp', fp)


class _Utf8:
    def __init__(self, b): self.b = b
    def write(self, s): self.b.write(s.encode('utf-8'))


def part_path():
    """data/teksty/<дата>-NN.csv.gz. Номер завжди є: «2026-09-24-02» мусить
    сортуватися після «2026-09-24-01» — пізніша частина перемагає."""
    day = datetime.date.today().isoformat()
    k = 1
    while os.path.exists(os.path.join(TEKSTY, f'{day}-{k:02d}.csv.gz')):
        k += 1
    return os.path.join(TEKSTY, f'{day}-{k:02d}.csv.gz')


def main_pererakhuvaty():
    """Перерахунок частин зі старим правилом за чинним — без завантаження.
    Результат — нова частина з RULE; старі лишаються як історія."""
    old = {}
    for fp in sorted(glob.glob(os.path.join(TEKSTY, '*.csv.gz'))):
        with gzip.open(fp, 'rt', encoding='utf-8', newline='') as fh:
            for r in csv.DictReader(fh, delimiter='\t'):
                if r.get('rule') != RULE:
                    old[r['doc_id']] = r
    done = load_done()
    todo = {d: r for d, r in old.items() if d not in done}
    print(f'до перерахунку: {len(todo):,} (уже за правилом {RULE}: {len(done):,})')
    if not todo:
        return
    # вид справи в частині не записано — беремо з events: від нього залежить
    # правило місця (наркотичні, v3)
    with gzip.open(os.path.join(DATA, 'events.csv.gz'), 'rt', encoding='utf-8', newline='') as fh:
        cat = {r['doc_id']: r['cat'] for r in csv.DictReader(fh, delimiter='\t') if r.get('doc_id') in todo}
    out = {d: pererakhuvaty(r, cat.get(d, '')) for d, r in todo.items()}
    fp = part_path()
    _write(fp, out)
    ch = collections.Counter()
    for d, r in out.items():
        o = todo[d]
        ch['рівень ' + o['level'] + ' -> ' + r['level']] += 1 if o['level'] != r['level'] else 0
        ch['позначка ' + (o['vada'] or '-') + ' -> ' + (r['vada'] or '-')] += 1 if o['vada'] != r['vada'] else 0
        ch['дата змінилася'] += o['date'] != r['date']
    print(f'   {os.path.relpath(fp, ROOT)}: {len(out):,}')
    for k, n in ch.most_common():
        if n: print(f'   {k}: {n:,}')


def main():
    if '--pererakhuvaty' in sys.argv:
        return main_pererakhuvaty()
    budget = int(os.environ.get('MAX_DOCS', '30000') or 0)
    done = load_done()
    reps, refs = queue_docs()
    todo = [r for r in reps if r['doc_id'] not in done and r['doc_id'] in refs]
    no_ref = sum(1 for r in reps if r['doc_id'] not in done and r['doc_id'] not in refs)
    print(f'представників справ: {len(reps):,}; уже розібрано: {len(done):,}; '
          f'у черзі: {len(todo):,}; без посилання на текст: {no_ref:,}')
    if budget and len(todo) > budget:
        todo = todo[:budget]
    if not todo:
        print('черга порожня'); return
    print('цього запуску: ' + ', '.join(f'{t} {n:,}' for t, n in
                                       collections.Counter(PD.theme(r['cat']) for r in todo).most_common()))
    fp = part_path()
    q = queue.Queue(); [q.put(r) for r in todo]
    out, lock, err = {}, threading.Lock(), [0]
    t0 = time.time()

    def worker():
        while True:
            try: r = q.get_nowait()
            except queue.Empty: return
            ref = refs[r['doc_id']]
            res = None
            for attempt in range(3):
                try:
                    rq = urllib.request.Request(REESTR + ref[:2] + '/' + ref[2:] + '.rtf',
                                                headers={'User-Agent': UA})
                    with urllib.request.urlopen(rq, timeout=45) as resp:
                        res = rozbir(rtf_to_text(resp.read()), r['cat'])
                    break
                except Exception:
                    time.sleep(1.5 * (attempt + 1))
            with lock:
                if res is None:
                    err[0] += 1          # не записуємо: наступний запуск спробує знову
                else:
                    res = {k: ('1' if v is True else '0' if v is False else v) for k, v in res.items()}
                    res.update(doc_id=r['doc_id'], rule=RULE)
                    out[r['doc_id']] = res
                n = len(out) + err[0]
                if n % 200 == 0:
                    sp = n / max(time.time() - t0, 1)
                    print(f'   {n:,} з {len(todo):,} · {sp:.1f}/с · помилок {err[0]}', flush=True)
                if len(out) % SAVE_EVERY == 0 and out:
                    _write(fp, out)
            time.sleep(DELAY)

    ths = [threading.Thread(target=worker, daemon=True) for _ in range(WORKERS)]
    for t in ths: t.start()
    try:
        for t in ths: t.join()
    except KeyboardInterrupt:
        print('перервано — зроблене збережено')
    with lock:
        if out:
            _write(fp, out)
    mins = (time.time() - t0) / 60
    print(f'\n=== ГОТОВО за {mins:.0f} хв: {len(out):,} розібрано, помилок {err[0]} ===')
    if out:
        print(f'   {os.path.relpath(fp, ROOT)} — {os.path.getsize(fp) / 1048576:.1f} МБ')
        th = {r['doc_id']: PD.theme(r['cat']) for r in todo}
        by = collections.defaultdict(collections.Counter)
        for d, v in out.items():
            by[th[d]][v['vada'] or 'добра'] += 1
            by[th[d]]['клас ' + v['klass']] += 1
        for t in ORDER:
            if t in by: print(f'   {t}: ' + ', '.join(f'{k} {n:,}' for k, n in sorted(by[t].items())))


if __name__ == '__main__':
    main()
